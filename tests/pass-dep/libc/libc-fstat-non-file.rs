...
fn test_fstat_stdin() {
    let mut buf = MaybeUninit::uninit();
    let stat = do_fstat(libc::STDIN_FILENO, &mut buf);
    assert_eq!(stat.st_mode & libc::S_IFMT, libc::S_IFCHR, "stdin should be S_IFCHR");
    assert_stat_fields_are_accessible(stat);
}
...
fn test_fstat_stdout() {
    let mut buf = MaybeUninit::uninit();
    let stat = do_fstat(libc::STDOUT_FILENO, &mut buf);
    assert_eq!(stat.st_mode & libc::S_IFMT, libc::S_IFCHR, "stdout should be S_IFCHR");
    assert_stat_fields_are_accessible(stat);
}
...
fn test_fstat_stderr() {
    let mut buf = MaybeUninit::uninit();
    let stat = do_fstat(libc::STDERR_FILENO, &mut buf);
    assert_eq!(stat.st_mode & libc::S_IFMT, libc::S_IFCHR, "stderr should be S_IFCHR");
    assert_stat_fields_are_accessible(stat);
}
